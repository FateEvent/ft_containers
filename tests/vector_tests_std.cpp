...
int main() {
	...
	std::stack<Buffer, std::vector<Buffer> > stack_deq_buffer;
	...
	std::vector<char>	newVector(itLetter, itLetter2);

	vector_print(newVector);

	newVector.push_back('j');
	for (std::vector<char>::iterator it = newVector.end() - 2; it != newVector.end(); it++)
	...
	std::vector<char>	newDeck(itLetter, itLetter2);
